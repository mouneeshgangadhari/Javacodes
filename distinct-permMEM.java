...
class Main
{
	public static int findPermutations(int[] nums, int target, int[] T)
	{
		...
		// if the subproblem is solved before, return the saved result
		if (T[target] != Integer.MIN_VALUE) {
			return T[target];
		}

		// check if target can be reached by including each number
		int count = 0;
		for (int i : nums) {
			if(target - i >= 0)
				count += findPermutations(nums, target - i, T);
		}

		// save and return solution to the current subproblem
		T[target] = count;
